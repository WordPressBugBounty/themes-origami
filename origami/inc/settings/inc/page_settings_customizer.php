	...
	public function customize_register( $wp_customize ) {
		...
		if ( method_exists( $wp_customize, 'add_panel' ) ) {
			$wp_customize->add_panel( 'page_settings', array(
				'title' => __( 'Page Template Settings', 'origami' ),
				'description' => __( 'Change layouts for various pages on your site.', 'origami' ),
				'priority' => 11,
			) );
		}
		...
		$types = array();
		$templates = array(
			'home' => __( 'Blog Page', 'origami' ),
			'search' => __( 'Search Results', 'origami' ),
			'date' => __( 'Date Archives', 'origami' ),
			'author' => __( 'Author Archives', 'origami' ),
			'404' => __( 'Not Found', 'origami' ),
		);
		...
		foreach ( $post_types as $post_type => $post_type_data ) {
			...
			$types[] = array(
				'group' => 'archive',
				'id' => $post_type,
				'title' => __( 'Type', 'origami' ) . ': ' . $post_type_data->label,
			);
		}
		...
		foreach ( $taxonomies as $tax_slug => $taxonomy ) {
			...
			$types[] = array(
				'group' => 'taxonomy',
				'id' => $tax_slug,
				'title' => __( 'Taxonomy', 'origami' ) . ': ' . $taxonomy->label,
			);
		}
